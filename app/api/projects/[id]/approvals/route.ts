...
export async function GET(_request: Request, { params }: Context) {
  ...
  return Response.json({ approvals: listApprovals(id) });
}
...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    return Response.json({ approval: createApproval(id, { stage: body.stage.trim(), status: body.status, note: body.note }, user.email) }, { status: 201 });
  } catch (error) {
