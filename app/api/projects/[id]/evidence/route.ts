...
export async function GET(_request: Request, { params }: Context) {
  ...
  return Response.json({ evidence: listEvidence(id) });
}
...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    const evidence = createEvidence(id, { sourceId: body.sourceId, claim: body.claim.trim(), confidence: body.confidence, classification: body.classification }, user.email);
