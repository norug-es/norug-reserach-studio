...
export async function GET(_request: Request, { params }: Context) {
  ...
  return Response.json({ evidence: await listEvidence(context, id) });
}
...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    const evidence = await createEvidence(context, id, { sourceId: body.sourceId, claim: body.claim.trim(), confidence: body.confidence, classification: body.classification }, user.email);
