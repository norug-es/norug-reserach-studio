...
import { closeDb, ensureDatabase, withTenantTransaction } from "../lib/db.ts";

type Row = Record<string, string | number | null>;

const sourcePath = resolve(process.cwd(), process.env.SQLITE_PATH ?? "./data/research-studio.db");
const tenantId = process.env.IMPORT_TENANT_ID ?? "workspace-norug-lab";
const userId = process.env.IMPORT_USER_ID ?? "user-admin";
  ...
  await withTenantTransaction({ tenantId, userId }, async (client) => {
    for (const row of projects) {
      await client.query(`INSERT INTO projects
        (id, tenant_id, name, area, language, output, status, progress, human_approval, created_at, updated_at)
        ...
        [row.id, tenantId, row.name, row.area, row.language, row.output, row.status, row.progress,
          Boolean(row.human_approval), row.created_at, row.updated_at],
      );
    }
    for (const row of sources) {
      await client.query(`INSERT INTO sources
        (id, tenant_id, project_id, type, title, url, status, confidence, created_at)
        ...
        [row.id, tenantId, row.project_id, row.type, row.title, row.url, row.status, row.confidence, row.created_at],
      );
    }
    for (const row of evidence) {
      await client.query(`INSERT INTO evidence
        (id, tenant_id, project_id, source_id, claim, classification, confidence, sha256, created_at)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
        ...
        [row.id, tenantId, row.project_id, row.source_id, row.claim, row.classification,
          row.confidence, row.sha256, row.created_at],
      );
    }
    for (const row of approvals) {
      await client.query(`INSERT INTO approvals
        (id, tenant_id, project_id, stage, status, reviewer, note, created_at)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
        ON CONFLICT (id) DO UPDATE SET project_id=EXCLUDED.project_id,
          stage=EXCLUDED.stage, status=EXCLUDED.status, reviewer=EXCLUDED.reviewer,
          note=EXCLUDED.note`,
        [row.id, tenantId, row.project_id, row.stage, row.status, row.reviewer, row.note, row.created_at],
      );
    }
    for (const row of activity) {
      await client.query(`INSERT INTO activity
        (id, tenant_id, project_id, action, detail, actor, created_at)
        VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (id) DO NOTHING`,
        [row.id, tenantId, row.project_id, row.action, row.detail, row.actor, row.created_at],
      );
    }
  });
