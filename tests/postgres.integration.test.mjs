...
test("PostgreSQL aplica migraciones, conserva datos y fuerza aislamiento RLS", async () => {
  ...
  try {
    ...
    assert.equal(health.migrationVersion, 9);
    ...
    const identitySchema = await database.query(`SELECT
      ...
      to_regclass('public.evidence_bundles') IS NOT NULL AS "bundlesTable",
      to_regclass('public.evidence_bundle_entries') IS NOT NULL AS "bundleEntriesTable",
      (
        ...
        FROM pg_class WHERE relname IN ('stored_objects', 'processing_jobs',
          ...
          'transcriptions', 'transcription_segments', 'evidence_bundles',
    ...
    assert.equal(identitySchema.rows[0].bundlesTable, true);
    assert.equal(identitySchema.rows[0].bundleEntriesTable, true);
