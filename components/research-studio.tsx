...
import type { ExtractedDocumentSummary, ProcessingJob, ProjectSnapshot, ResearchProject, SecurityScan, SessionUser, StoredObject, TranscriptionDetail, TranscriptionSummary, Workspace } from "@/lib/types";
...
export function ResearchStudio({ user, workspaces, projects: initialProjects, initialSnapshot }: Props) {
  ...
  return <div className="shell">
    <aside>
      <div className="brand"><strong>N<span>R</span></strong><b>Research Studio<small>by norug.es · v0.6.4</small></b></div>
    ...
    <main>
      ...
      {tab === "Ingesta" && <IngestionPanel objects={objects} jobs={jobs} scans={scans} documents={documents} transcriptions={transcriptions} canEdit={canEdit} busy={busy} onUpload={() => setModal("upload")} onRetry={retryJob} onOpenTranscript={setTranscriptObject}/>} 
    ...
    {modal === "upload" && <Modal onClose={() => setModal(null)}><form onSubmit={uploadFile}><em>INGESTA SEGURA</em><h2>Sube material de investigación</h2><p>El archivo se deduplica, verifica por firma binaria, analiza con ClamAV y después se extrae o transcribe.</p><label>Archivo<input name="file" type="file" accept=".pdf,.docx,.txt,.md,.csv,.mp3,.wav,.m4a,.mp4,.mpeg,.mpg,.webm,.mov" required/></label><small className="upload-help">Los documentos generan texto y fragmentos. Audio y vídeo —incluido MPEG/MPG— se transcriben con Whisper, idioma automático y timestamps. La vista previa depende de los códecs que soporte el navegador.</small><button className="primary submit" disabled={busy}>{busy ? "Subiendo…" : "Escanear y procesar"}</button></form></Modal>}
...
function IngestionPanel({ objects, jobs, scans, documents, transcriptions, canEdit, busy, onUpload, onRetry, onOpenTranscript }: {
