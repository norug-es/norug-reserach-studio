...
import type { ProcessingJob, ProjectSnapshot, ResearchProject, SessionUser, StoredObject, Workspace } from "@/lib/types";
...
export function ResearchStudio({ user, workspaces, projects: initialProjects, initialSnapshot }: Props) {
  ...
  const { project, sources, evidence, approvals, activity, objects, jobs } = snapshot;
  ...
  return <div className="shell">
    <aside>
      <div className="brand"><strong>N<span>R</span></strong><b>Research Studio<small>by norug.es · v0.6.0</small></b></div>
    ...
    <main>
      ...
      {tab === "Ingesta" && <IngestionPanel objects={objects} jobs={jobs} canEdit={canEdit} busy={busy} onUpload={() => setModal("upload")} onRetry={retryJob}/>} 
    ...
    {modal === "upload" && <Modal onClose={() => setModal(null)}><form onSubmit={uploadFile}><em>INGESTA CONTROLADA</em><h2>Sube material de investigación</h2><p>El archivo se almacena de forma privada, se deduplica por SHA-256 y pasa al worker para verificar su integridad.</p><label>Archivo<input name="file" type="file" accept=".pdf,.docx,.txt,.md,.csv,.mp3,.wav,.m4a,.mp4,.webm,.mov" required/></label><small className="upload-help">PDF, DOCX, TXT, MD, CSV, MP3, WAV, M4A, MP4, WEBM o MOV · máximo 50 MB por defecto. El análisis antimalware llegará en v0.6.1.</small><button className="primary submit" disabled={busy}>{busy ? "Subiendo…" : "Almacenar y procesar"}</button></form></Modal>}
...
function IngestionPanel({ objects, jobs, canEdit, busy, onUpload, onRetry }: {
  objects: StoredObject[]; jobs: ProcessingJob[]; canEdit: boolean; busy: boolean;
  onUpload: () => void; onRetry: (jobId: string) => void;
}) {
  const jobByObject = new Map(jobs.map((job) => [job.objectId, job]));
  return <section className="panel evidence ingestion-panel"><div className="title"><div><h2>Ingesta y procesamiento</h2><p>Objetos privados · deduplicación SHA-256 · jobs idempotentes · reintentos exponenciales.</p></div>{canEdit && <button onClick={onUpload}>＋ Subir archivo</button>}</div>{objects.length === 0 ? <div className="ingestion-empty"><b>Sin objetos almacenados</b><p>Sube un documento, audio o vídeo para iniciar el pipeline.</p></div> : <div className="object-list">{objects.map((object) => { const job = jobByObject.get(object.id); return <article key={object.id}><i>{object.contentType.startsWith("video/") ? "VID" : object.contentType.startsWith("audio/") ? "AUD" : "DOC"}</i><div><h3>{object.originalName}</h3><p>{formatBytes(object.sizeBytes)} · SHA-256 {object.sha256.slice(0, 16)}…</p><u><b style={{ width: `${job?.progress ?? (object.status === "ready" ? 100 : 0)}%` }}/></u></div><div className="object-state"><em className={object.status}>{object.status}</em><small>{job ? `${job.status} · intento ${job.attempts}/${job.maxAttempts}` : "sin trabajo"}</small></div><a href={`/api/objects/${object.id}/download`}>Descargar</a>{job && ["failed", "dead_letter"].includes(job.status) && canEdit && <button disabled={busy} onClick={() => onRetry(job.id)}>Reintentar</button>}</article>; })}</div>}</section>;
}
