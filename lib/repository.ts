...
import { listBundleEntries, listExtractedDocuments, listProcessingJobs, listSecurityScans, listStoredObjects, listTranscriptions } from "@/lib/ingestion";
...
export async function getProjectSnapshot(context: TenantContext, id: string): Promise<ProjectSnapshot | null> {
  ...
  const [sources, evidence, approvals, activity, objects, jobs, scans, documents, transcriptions, bundleEntries] = await Promise.all([
  ...
  return { project, sources, evidence, approvals, activity, objects, jobs, scans, documents, transcriptions, bundleEntries };
}
